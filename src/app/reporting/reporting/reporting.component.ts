...
countOccurrences(items: (string | undefined)[]): { [key: string]: number } {
 ...
 checkFormationPeriod(emp: EmployeReportingDTO, [start, end]: [Date, Date]): boolean {
  if (!emp.periodesFormations) return false;

  const periodes = emp.periodesFormations.split(',').map(p => p.trim());

  return periodes.some(periode => {
    const [debutStr, finStr] = periode.split(' au ').map(d => d.trim());
    const debut = new Date(debutStr);
    const fin = new Date(finStr);

    if (isNaN(debut.getTime()) || isNaN(fin.getTime())) return false;

    // Vérifie si la période de formation chevauche la période filtrée
...
reportTypes = [
  ...
  { label: 'Par niveau de diplôme', value: 'diplome' },
  { label: 'Par société d\'expérience', value: 'societe' }
];
